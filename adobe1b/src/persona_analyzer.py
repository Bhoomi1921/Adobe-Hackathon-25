...
class PersonaAnalyzer:
    ...
    def __init__(self):
        ...
        self.job_types = {
            ...
            'decision': ['decide', 'choose', 'select', 'determine', 'recommend'],
            'comparison': ['compare', 'contrast', 'benchmark', 'evaluate']
        }
    ...
    def _define_success_criteria(self, job_description: str) -> List[str]:
        ...
        if any(word in job_description.lower() for word in ['summary', 'overview']):
            criteria.append('Concise summary of key points')
        
        if any(word in job_description.lower() for word in ['recommendation', 'suggestion']):
            criteria.append('Actionable recommendations')
